...
export default function Qualities() {
  const allqualities = [
    {
      icon: <img className="h-auto w-14" src={`${process.env.PUBLIC_URL}/images/icecream/IceCream.svg`} alt="icon" />,
  ...
  return (
    <section className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white body-font">
      <div className="container px-5 py-8 mx-auto">
        <div className="text-center mb-14">
          <h1 className="sm:text-4xl text-2xl font-medium title-font mb-4">Our Signature <span className="text-cream-600">Qualities</span></h1>
        ...
        <div className="flex flex-wrap sm:-m-4 -mx-4 -mb-10 -mt-4 md:space-y-0 space-y-6">
          {
            allqualities.map((item, index) => (
              <div key={index} className="p-4 md:w-1/3 flex flex-col text-center items-center">
                <div className="w-20 h-20 inline-flex items-center justify-center rounded-full bg-cream-200 text-cream-600 mb-5 flex-shrink-0">
                  {item.icon}
                </div>
