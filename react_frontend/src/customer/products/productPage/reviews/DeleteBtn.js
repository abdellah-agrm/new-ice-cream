...
export default function DeleteBtn({ reviewID }) {
  ...
  const confirmDelete = () => {
    toast((t) => (
      <div className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white relative p-4 text-center rounded-lg shadow sm:p-5">
        ...
        <div className="flex justify-center items-center space-x-4">
          <button onClick={() => deleteit() && toast.dismiss(t.id)} type="submit" className="py-2 px-3 text-sm font-medium text-center text-white bg-cream-600 rounded-lg hover:bg-cream-500">
            Yes, I'm sure
          </button>
  ...
  return (
    <button onClick={confirmDelete} className="bg-cream-600 text-white hover:bg-cream-600/90 p-1.5 rounded">
      <Trash className="h-5 w-auto" />
    </button>
  )
}