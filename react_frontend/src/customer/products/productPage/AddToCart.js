...
export default function AddToCart({ order }) {
  ...
  return (
    <>
      <Toaster />
      <button onClick={() => setShowModal(true)} className="flex ml-auto items-center text-white bg-cream-600 hover:bg-cream-600/90 rounded-md px-3 py-2 text-center text-sm font-medium outline-none disabled:text-cream-400" 
      disabled={order.ProductStock < 1 || userRole === "vendor"}>
        <ShoppingCartIcon className="mr-2 h-6 w-6" />
        Add to cart
      </button>
      {showModal ? (
        <div className="bg-cream-600/80 overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
          <div className="relative animate-fade-up animate-duration-[1500ms] animate-ease-out w-auto my-6 mx-auto max-w-3xl">
            <div className="mx-auto max-w-md pt-20">
              <div className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white rounded-xl shadow-lg">
                <div className="px-4 py-4 sm:px-5 sm:py-6">
                  <div className="flow-root">
                    <ul className="-my-8">
                      <li className="flex flex-col space-y-3 py-6 text-left sm:flex-row sm:space-x-5 sm:space-y-0">
                        <div className="shrink-0 relative">
                          <span className="absolute -top-2 -left-2 flex h-6 w-6 items-center justify-center rounded-full bg-cream-500 text-white text-sm font-medium shadow">{quantity}</span>
                          ...
                          <button onClick={() => setShowModal(false)} type="button" className="text-gray-500 hover:text-gray-900 absolute top-1 right-1 block sm:hidden text-base font-semibold text-right rounded transition-all duration-200 ease-in-out focus:shadow">
                            <X className="h-6 w-6" />
                          </button>
                        </div>
                        ...
                        <div className="relative flex flex-1 flex-col justify-between">
                          <div className="sm:col-gap-5 sm:grid sm:grid-cols-2">
                            <div className="p-0">
                              <p className="text-base font-semibold">{order.ProductName}</p>
                              <p className="mx-0 mt-1 mb-0 text-base font-semibold">${order.ProductPrice}</p>

                              <div className="flex flex-row h-6 w-full rounded-lg relative bg-transparent mt-1">
                                <input type="number" value={quantity} onChange={(e) => setQuantity(e.target.value)} min={0} className="quantity bg-white dark:bg-gray-900 text-gray-900 dark:text-white p-0 w-11 border-0 flex items-center focus:outline-none outline-none focus:ring-0 focus:border-transparent text-center font-semibold text-md hover:text-black focus:text-black" />
                  ...
                  <div className="mt-6 space-y-3 border-t border-b py-8">
                    <div className="flex items-center justify-between">
                      <p className="text-gray-600 dark:text-gray-200">Stock</p>
                      <p className="text-lg font-semibold">{parseInt(order.ProductStock) - quantity}</p>
                    </div>
                    <div className="flex items-center justify-between">
                      <p className="text-gray-600 dark:text-gray-200">Quantity</p>
                      <p className="text-lg font-semibold">{quantity}</p>
                    </div>
                  </div>
                  <div className="mt-6 flex items-center justify-between">
                    <p className="text-sm font-medium">Total</p>
                    <p className="text-2xl font-semibold"><span className="text-xs font-normal text-gray-600 dark:text-gray-200">USD</span> {(quantity * order.ProductPrice).toFixed(2)}</p>
                  </div>
