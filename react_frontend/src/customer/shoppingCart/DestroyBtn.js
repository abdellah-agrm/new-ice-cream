...
export default function DestroyBtn({ User_id, Product_id, Refresh }) {
  ...
  const confirmDelete = (User_id, Product_id) => {
    toast((t) => (
      <div className="relative bg-white dark:bg-gray-900 text-gray-900 dark:text-white p-4 text-center rounded-lg shadow sm:p-5">
        <button onClick={() => toast.dismiss(t.id)} type="button" className="absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1 ml-auto inline-flex items-center">
          <X className="w-6 h-6" />
        </button>
