...
import { LoadingPage2 } from "../../Elements/LoadingPage";
...
export default function ShoppingCart({ CloseSideBar }) {
  ...
  return (
    <div className="ShoppingCart_div bg-white dark:bg-gray-900 text-gray-900 dark:text-white font-poppins fixed top-0 bottom-0 right-0 z-10 w-full max-w-lg overflow-y-scroll border border-cream-600 rounded">
      <div className="p-6 md:pt-4 md:pb-6 md:px-6">
          ...
          <div key={index} className="block pb-6 mb-6 -mx-4 border-b border-gray-200 md:flex">
            ...
            <div className="w-full py-0 md:py-0 px-4 md:px-3 md:2/3">
              ...
              <div className="flex justify-between text-sm mt-5">
                <div className="">
                  <p className="flex items-end font-medium">Price :</p>
                  <p className="flex items-end font-medium">Quantity :</p>
                  <p className="flex items-end font-medium">Total :</p>
                </div>
                <div className="">
                  <p className="flex items-end font-medium">${item.ProductPrice.toFixed(2)}</p>
                  <p className="flex items-end font-medium">{item.TotalQuantity}</p>
                  <p className="flex items-end font-medium">${item.AllTotalAmount.toFixed(2)}</p>
                </div>
              </div>
            </div>
          </div>
