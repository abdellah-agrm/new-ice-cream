  ...
  const card = [
    {
      ...
      'icon': <img className="w-12 h-auto" src={`${process.env.PUBLIC_URL}/images/icecream/icecream_icon.png`} alt="icon" />
    }, {
  ...
  return (
    <div className="px-14 pt-6">
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
        {
          card.map((item, index) => (
            <div key={index} className="p-4 dark:bg-gray-800 border border-cream-300 rounded-lg shadow hover:shadow-md">
              <div className="flex flex-row items-center">
                <div className="flex-1 text-left md:text-left">
                  <h2 className="mb-2 text-lg font-bold text-cream-600 uppercase">{item.title}</h2>
                  <p className="mb-2 text-lg font-medium">{item.stats}</p>
                </div>
                <div className="flex-shrink">
                  <div className="flex items-center pl-8 pr-2 py-4 text-cream-600">
